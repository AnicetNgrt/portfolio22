...
Which obviously turns huge part of your APIs into a mess in my opinion. And often it isn't even as permissive as you would hope for that sheer amount of added complexity. There are more clever tricks than that of course, but to me it always feels cluttered in the end. Zig would be perfect for that as I said, but in Rust we'll just hope the Data Scientists know what size the layers in their networks should be. Which actually is a crucial part of their job, and certainly not the one they need help with. Of course for the guy like me implementing the framework, it would be super cool to have such compile-time checks. I truly have suffered from not having them while debugging some of the internals, but figuring out how to avoid bugs without these compile-ime guarantees is my part of the job, and I'll do it harder anyway if it is better in the end for my users.
